...
class RBFN(object):
    ...
    def _kernel_function(self, center, data_point):
        return np.exp(-0.5/self.sigma*np.linalg.norm(center-data_point)**2)
    ...
    def fit(self,X,Y):
        ...
        """
        random_args = np.random.permutation(X.shape[0]).tolist() #返回一个输入长度的随机列表
        self.centers = [X[arg] for arg in random_args][:self.hidden_shape] #取前hidden_shape个数据
        G = self._calculate_interpolation_matrix(X)
        self.weights = np.dot(np.linalg.pinv(G),Y)#G的伪逆与Y的矩阵相乘
