...
def batch_generator(data, batch_size, time_step):
    batch_count = 0
    while True:
        if batch_count * batch_size * time_step + batch_size * time_step \
            >= data[0].shape[0]:
            batch_count = 0
        start = batch_count * batch_size * time_step
        end = start + batch_size * time_step
        batch_count += 1
        yield [d[start:end].reshape(batch_size, time_step) for d in data]
